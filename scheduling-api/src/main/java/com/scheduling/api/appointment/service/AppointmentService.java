...
public class AppointmentService {
    ...
    public AppointmentResponse create(CreateAppointmentRequest req, User currentUser) {
        if (currentUser.getRole() == Role.PROFESSIONAL) {
            throw new AccessDeniedException("Profissionais não podem criar agendamentos diretamente.");
        }
        ...
        AppointmentStatus status = (currentUser.getRole() == Role.ADMIN
                ...
                : AppointmentStatus.PENDING;   // CLIENT → PENDING
    ...
    public Page<AppointmentResponse> findByCompany(Long companyId, LocalDateTime start,
                                                    ...
                                                    User currentUser, Long professionalId) {
        assertCompanyAccess(currentUser, companyId);
        // PROFESSIONAL sempre vê apenas seus próprios agendamentos
        Long effectiveProfId = currentUser.getRole() == Role.PROFESSIONAL
                ? currentUser.getId()
                : professionalId;

        if (effectiveProfId != null) {
            return appointmentRepository
                    .findByCompanyAndProfessionalInPeriod(companyId, effectiveProfId, start, end, pageable)
                    .map(this::toResponse);
        }
        return appointmentRepository
    ...
    public List<AppointmentResponse> findPendingByCompany(Long companyId, User currentUser, Long professionalId) {
        assertCompanyAccess(currentUser, companyId);
        Long effectiveProfId = currentUser.getRole() == Role.PROFESSIONAL
                ? currentUser.getId()
                : professionalId;
        ...
        if (effectiveProfId != null) {
            return appointmentRepository
                    .findPendingByCompanyAndProfessional(companyId, AppointmentStatus.PENDING, effectiveProfId)
    ...
    public AppointmentResponse cancel(Long id, String reason, User currentUser) {
        ...
        switch (currentUser.getRole()) {
            ...
            case PROFESSIONAL -> throw new AccessDeniedException("Profissionais não podem cancelar agendamentos.");
    ...
    public AppointmentResponse reschedule(Long id, RescheduleRequest req, User currentUser) {
        ...
        switch (currentUser.getRole()) {
            ...
            case MANAGER -> assertCompanyAccess(currentUser, a.getCompany().getId());
            case PROFESSIONAL -> throw new AccessDeniedException("Profissionais não podem remarcar agendamentos.");
            default -> {}
        }
    ...
    /**
     * ADMIN → passa sempre.
     * MANAGER / PROFESSIONAL → company deve estar no set de empresas do usuário.
     */
    private void assertCompanyAccess(User currentUser, Long companyId) {
        if (currentUser.getRole() == Role.ADMIN) return;
        if (currentUser.getRole() == Role.MANAGER || currentUser.getRole() == Role.PROFESSIONAL) {
            boolean hasAccess = currentUser.getCompanies().stream()
                    .anyMatch(c -> companyId.equals(c.getId()))
                    || (currentUser.getCompany() != null
                            && companyId.equals(currentUser.getCompany().getId()));
            if (!hasAccess) {
                throw new AccessDeniedException(
                        "Acesso negado: você só pode acessar agendamentos da sua empresa.");
            }
            return;
        }
