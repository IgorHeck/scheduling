...
public class ScheduleService {
    ...
    public ScheduleBlockResponse createBlock(ScheduleBlockRequest req, User currentUser) {
        assertCompanyAccess(currentUser, req.getCompanyId());
        Company company = companyService.findCompanyById(req.getCompanyId());
        User professional = req.getProfessionalId() != null
                ? userService.findUserById(req.getProfessionalId())
                : null;
    ...
    public List<ScheduleBlockResponse> findBlocks(Long companyId, Long professionalId, User currentUser) {
        assertCompanyAccess(currentUser, companyId);
        List<ScheduleBlock> blocks = professionalId != null
                ? blockRepository.findByCompanyIdAndProfessionalIdOrNull(companyId, professionalId)
                : blockRepository.findByCompanyId(companyId);
    ...
    private ScheduleBlockResponse toBlockResponse(ScheduleBlock b) {
        return ScheduleBlockResponse.builder()
                ...
                .endAt(b.getEndAt())
                .reason(b.getReason())
                .professionalId(b.getProfessional() != null ? b.getProfessional().getId() : null)
                .professionalName(b.getProfessional() != null ? b.getProfessional().getName() : null)
                .build();
    }
